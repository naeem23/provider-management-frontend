...
export default function CreateSpecialistPage() {
  ...
  const fetchSpecialistData = async () => {
    try {
      const response = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_BASE_URL}/specialists/specialists/${userId}/`)
      ...
      if (response.ok) {
        ...
        setFormData({
            ...
            avgDailyRate: data.avg_daily_rate,
            status: data.status,
            availableFromDate: data.available_from,
            availableUntilDate: data.available_until || '',
        ...
        setApiError('Failed to load specialist data')
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        available_until: formData.availableUntilDate,
      ...
      const response = await fetchWithAuth(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/specialists/specialists/${userId}/`, 
        {
            method: 'PUT',
      ...
      if (response.ok) {
        ...
        setApiError(errorData.detail || errorData.message || 'Failed to update specialist')
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Update Specialist</h1>
        <p className="text-gray-600 mt-2">Edit specialist information</p>
      </div>
